...
from .layers import get_inputs, get_embedding, TokenEmbedding, EmbeddingSimilarity, Masked, Extract, get_rec_inputs, get_rec_embedding
...
TOKEN_PAD = 0  # Token for padding
TOKEN_UNK = 1
TOKEN_CLS = 2  # Token for classification
TOKEN_SEP = 3  # Token for separation
TOKEN_MASK = 4  # Token for masking
...
def get_rec_model(token_num,
    ...
    inputs = get_rec_inputs(seq_len=seq_len) # sequence and masks only
    embed_layer, embed_weights = get_rec_embedding(
        inputs,
        token_num=token_num,
        embed_dim=embed_dim,
        pos_num=pos_num,
...
def gen_batch_inputs(sentence_pairs,
    ...
    for i in range(batch_size):
        first, second = sentence_pairs[i][0], [sentence_pairs[i][1]]
        # print(f"sentence_pairs:{first}{second}")

        segment_inputs.append(([0] * (len(first) + 2) + [1] * (seq_len - (len(first) + 2)))[:seq_len])
        tokens = first + [TOKEN_SEP] + second
        # print(f'tokens: {tokens}')
        tokens = tokens[:seq_len]
        tokens = list(tokens) + [TOKEN_PAD] * (seq_len - len(tokens))
...
def gen_rec_batch_inputs(sentences,
                     token_dict,
                     token_list,
                     seq_len=50,
                     mask_rate=0.15,
    ...
    for i in range(batch_size):
        ...
        # print(f'tokens: {tokens}')
        tokens = tokens[-seq_len:]
        seq_tokens = len(tokens)
        # if seq_tokens < seq_len:
        #     tokens += [token_dict[TOKEN_SEP]]
        tokens = list(tokens) + [TOKEN_PAD] * (seq_len - len(tokens))
        ...
        has_mask = False
        if force_last:
            mlm_output = [token_dict.get(x, unknown_index) for x in tokens]
            masked_input = [0] * seq_len
            try:
                masked_input[seq_tokens-1] = 1
            except:
                raise Exception(f'wat tha heck {seq_tokens}')

            token_input = [token_dict.get(x, unknown_index) for x in tokens]
            token_input[seq_tokens-1] = token_dict[TOKEN_MASK]
        ...
        masked_inputs.append(masked_input)
        mlm_outputs.append(mlm_output)
    inputs = [np.asarray(x) for x in [token_inputs, masked_inputs]]
    if train:
        outputs = [np.asarray(np.expand_dims(x, axis=-1)) for x in [mlm_outputs]]
